...
从运行时实现可以直接看到认证和 OAuth 相关端点与 scope：
...
- 交付物里带了多平台音频原生模块
- runtime 里存在 `voiceEnabled` 之类的开关和相关字段

这足以稳定推出：

**Claude Code 运行时预留了非文本输入能力，并且语音不是纯粹的界面摆设。**

但如果把它直接写成“完整语音输入链路已经在每个环境里都成立”，那就超出了当前证据本身。
...
- Claude Code 已经把输入层做成了不只面向键盘文本的 runtime 形态
- 交付产物内明确存在语音相关实现与开关
- 至少可以稳定确认“非文本输入能力被认真建模了”
...
运行时实现顶部能直接看到一批 telemetry 指标名字，例如：
...
```python
def product_runtime(user_input):
    identity = resolve_auth_identity()
    normalized_input = collect_text_or_voice_input(user_input)
